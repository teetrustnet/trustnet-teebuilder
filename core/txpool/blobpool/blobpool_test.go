...
func TestAdd(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for j, add := range tt.adds {
			...
			if errs := pool.Add([]*types.Transaction{signed}, true, false); !errors.Is(errs[0], add.err) {
...
func TestSidecarConversion(t *testing.T) {
	...
	errs := pool.Add(preOsakaTxs, true, false)
	...
	go func() {
		pool.Add(types.Transactions{postOsakaTxs[0]}, false, false)
		close(addDone)
	}()

	// Add the post-Osaka v1 sidecar txs.
	errs = pool.Add(postOsakaTxs[1:], false, false)
	for _, err := range errs {
		if err != nil {
			t.Fatalf("expected tx add to succeed: %v", err)
		}
